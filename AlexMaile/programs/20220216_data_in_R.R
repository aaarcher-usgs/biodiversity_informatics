...
#' 
df.ex <- read.csv(file = "data/raw/Dataset_S1.txt", stringAsFactors = F)
...
#' 
colnames(df.ex) <- ifelse(test = colnames(df.ex) == "X.GC", yes = "percent.GC", no = colnames(df.ex))
...
#' 
#' spin this with:
ezspin(file = "AlexMaile/programs/20220216_data_in_R.R",out_dir = "AlexMaile/output", fig_dir = "figures",keep_md = FALSE, keep_rmd = FALSE)

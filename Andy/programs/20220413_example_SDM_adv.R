...
remove.IDs <- presences$uniqueID[presences$coordinateUncertaintyInMeters > 4000 &
...
#' 
remove.IDs.SE <- presences$uniqueID[presences$decimalLongitude < -88]
...
layers_choice <- unique(pred_layers[pred_layers$dataset_code == "Freshwater", c("name", "layer_code")])
layers_choice
layers_choice <- layers_choice[c(83, 4), ]
layers_choice
...
unique(sapply(layers[1], raster::extent))
...
layers <- lapply(layers, crop, extent(layers[[1:2]]))
...
plot(layers_cut[[1]], xlim = c(-92, -77), ylim = c(41, 44))
plot(pres_spat_vect, col = "blue", add = TRUE)
...
#' Map these data
plot(layers_cut[[1]], xlim = c(-88, -76), ylim = c(34, 44))
...
m1 <- sdm(presence ~ FW_dem_avg + FW_hydro_avg_01, 
          data = df.sdm, 
          methods = c("glm"))
m1
...
#' Variable selection?
#' 
m2.select <- sdm(presence ~ FW_dem_avg + I(FW_dem_avg^2) + FW_hydro_avg_01 + I(hydro_avg_01^2), 
...
#' Cross-validation
#' 
m3.cv <- sdm(presence ~ FW_dem_avg + FW_hydro_avg_01, 

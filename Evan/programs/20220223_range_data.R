...
#' Programmer: Evan
#' 
#' In this program, Some stuff about ranges!
#' 
#' 
#' ### Header
#' 
...
#' This is different than width, range summarizes the start and end and width across all the reads/ranges,
#' but width gives just each individual range's width



#' We can manipulate the ranges with standard arithmetic:
#' If you need to include a buffer or a primer to the beginning of a range, you
#' can change all of the values (x) at once!
(end(x) <- end(x) + 4)
x
...
#' In order to merge two ranges, make the ranges, then just use the "c" function!
...
#' paste0 is like a concaconate or whatever "c" function is called, then it removes
#' the space
...
#' This is helpful when we have primer or promoter ranges that come after 
x <- IRanges(start = c(40, 80),
             width = c(28, 35))
names(x) <- letters[1:length(x)]
x
plotIRanges(x)
#' Upstream flank
y <- flank(x, width = 7, start = T)
names(y) <- paste0(names(y), "-upstream")
#' Downstream flanks
z <- flank(x, width = 7, start = F)
names(z) <- paste0(names(z), "-downstream")
plotIRanges(c(x,y,z))
#' This is useful when we need to insert a promoter!
...
#' This gives us a random starting location with a width of 5!
...
#' 
#' spin this with:
#' ezspin(file = "Evan/programs/20220223_range_data.R",out_dir = "Evan/output", fig_dir = "figures20220223",keep_md = FALSE, keep_rmd = FALSE)

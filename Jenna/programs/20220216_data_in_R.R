...
#' Programmer: Jenna
#' 
#' In this program, we will continue our introduction with R (learning basic R 
#' tasks. Specifically, we will be working with data in R; reading in files and 
#' learning how to get summaries of the data.
#' 
#' 
#' ### Header
#' 
...
#' Add the second and thrid value of x to 4.7
#' 

x[c(2,3)] + 4.7
x[2:3] + 4.7 #colon reads as a sequence
...
x["coconut"] + 4.7 #goes through data until TRUE (banana FALSE)
...
(x <- x[order(x, decreasing = T)]) #if you just did x <- order(x, decreasing = T)
                                    #x would be replace by 2, 1, 5, 4 etc.
...
mean(x[x < 50 & x > 2], na.rm = T) #remove NA value
...
head(df.ex, n = 3) #number of rows to print
...
#' (R will always replace spaces with "." and "%" with X)

colnames(df.ex) <- ifelse(test = colnames(df.ex) == "X.GC",
                          yes = "percent.GC",
                          no = colnames(df.ex)) #or
...
mean(df.ex[ ,"depth"]) #need to account for all rows but only column "depth"
...
#' 
#' spin this with:
#' ezspin(file = "Jenna/programs/20220216_data_in_R.R",out_dir = "Jenna/output", fig_dir = "figures",keep_md = FALSE, keep_rmd = FALSE)

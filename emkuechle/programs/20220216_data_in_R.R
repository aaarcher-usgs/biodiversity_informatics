...
#' Programmer: Emma
#' 
#' In this program, in the first half we worked on learning more functions such 
#' as naming values in a vector. We then sorted the set in descending order 
#' and used the function mean.We then added a dataset and found means, then we 
#' created a new column to see if the values fell within the centromere.
#' 
#' 
#' ### Header
#' 
...
#' Query (find out) the column names for this dataset
#' 
colnames(df.ex)
...
#' 
df.ex$centromere <- NA # new column, just missing values
df.ex$centromere <- df.ex$start >= 25800000 & df.ex$end <= 29700000
